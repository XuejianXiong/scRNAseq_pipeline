...
# Generate consistent color map for treatment groups
treatment_palette = sns.color_palette("Set2", n_colors=10)  # adjust as needed
treatment_categories = None  # will be set after loading data
...
def plot_highly_variable_genes(adata, save_path="figures/04_hvg_colored.png"):
    """
    Plots mean vs dispersion highlighting highly variable genes (HVGs) in red.
    """
...
adata = sc.read("results/03_filtered_data.h5ad")
...
plot_highly_variable_genes(adata, save_path="figures/04_hvg_colored.png")
...
logging.info("Scaled the data to unit variance with max value 10")
...
logging.info("Computed neighbors and UMAP, saved plot colored by treatment")

# -----------------------------
# Save Processed Data
# -----------------------------

adata.write("results/04_norm_dr.h5ad")
logging.info("Saved normalized and reduced AnnData object to results/04_norm_dr.h5ad")

# -----------------------------
# Generate HTML Report
# -----------------------------

report_path = Path("results/04_report.html")
figures = sorted(Path(sc.settings.figdir).glob("*04_*.png"))
